...
- Signed data handling and fitting internally, rendered in absolute-value view
...
def load_filtered_dataset(
    ...
    if "kcu_actual_depower" in flight_data.columns:
        ...
        udp_signed = np.full_like(us_signed, np.nan, dtype=float)
...
def plot_heading_binned_panel(
    ...
    if np.any(scatter_mask):
        ax.scatter(
            np.abs(plot_df.loc[scatter_mask, "x"]),
            np.abs(plot_df.loc[scatter_mask, "y"]),
    ...
    if x_fit.size >= 2 and float(np.sum(x_fit**2)) > 1e-12:
        ...
        r2 = 1.0 - ss_res / ss_tot if ss_tot > 1e-12 else np.nan
        x_line_max = float(np.max(np.abs(x_fit)))
        x_line = np.linspace(0.0, x_line_max, 200)
        y_line = abs(slope) * x_line
...
def overlay_simulation_case(
    ...
    if fit_transient is not None:
        gk_t, r2_t, _, _, n_t = fit_transient
        summary = OverlayFitSummary(
            transient_gk=gk_t,
            transient_r2=r2_t,
    ...
    if fit_dynamic is not None:
        gk_d, r2_d, _, _, n_d = fit_dynamic
        summary = OverlayFitSummary(
...
def main() -> None:
    ...
    processed_data_dir = (
        Path(__file__).resolve().parent.parent / "ch9" / "processed_data"
    )
    for cfg, plot_df, fit_df in zip(datasets_cfg, plot_datasets, fit_datasets):
        ...
        if x_f.size >= 2 and float(np.sum(x_f**2)) > 1e-12:
            ...
            save_plot_data_to_csv(
                ...
                r2,
                processed_data_dir,
                addendum="_absolute_only",
            )

    # Edit these candidate lists to choose which simulation CSVs are used per panel.
    candidates_2019 = [
        # Path(__file__).resolve().parents[2] / "data" / "circles_batch_analysis_2019_5March.csv",
        # Path(__file__).resolve().parents[2] / "data" / "circles_batch_analysis_2019.csv",
        # Path(__file__).resolve().parents[2] / "data" / "circles_batch_analysis.csv",
    ]
    candidates_2025 = [
        # Path(__file__).resolve().parents[2] / "data" / "circles_batch_analysis_2025_5March.csv",
        # Path(__file__).resolve().parents[2] / "data" / "circles_batch_analysis_2025_3March.csv",
        # Path(__file__).resolve().parents[2] / "data" / "circles_batch_analysis_2025_3March_.csv",
    ]
    sim_datasets = [
    ...
    # Absolute-value view for all panels.
    ...
    for ax_idx, (ax, panel_entries, overlay_summary) in enumerate(
        ...
        combined_handles = list(fit_handles)
    ...
    output_path = (
        ...
        / "results"
        / "fitted_turn_rate_2019_and_2025_absolute_only.pdf"
    )
