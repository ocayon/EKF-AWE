...
velocity:
    ...
    offset  the legacy correction, a constant additive offset on the speed
    pitot   a calibration coefficient on the dynamic pressure, the way a pitot
            tube is actually calibrated

The flight, the configuration and the time window are selected from the
terminal, in the same way as run_analysis.py. The pre-processed flight data is
read directly from processed_data/, so the log has to be pre-processed already
(run run_analysis.py once if it is not).
"""
...
# Each variant is a set of overrides on the simulation_parameters of the config
VARIANTS = [
    ("none", "No calibration", {"calibrate_apparent_windspeed": False}),
    (
        "offset",
        "Offset (old)",
        {
            "calibrate_apparent_windspeed": True,
            "apparent_windspeed_calibration": "offset",
        },
    ),
    (
        "pitot",
        "Pitot calibration (new)",
        {
            "calibrate_apparent_windspeed": True,
...
def select_time_window(flight_data):
    """Ask for the time window to simulate, as run_analysis.py does."""
    ...
    print("Note: three full EKF runs are simulated, so keep the window short.")
    start_minute = int(
    ...
    if start_minute > 0 or end_minute > 0:
        if end_minute > 0 and end_minute <= len(flight_data) / 60 * dt:
            ...
            flight_data = flight_data.iloc[int(start_minute * 60 / dt) :].reset_index(
                drop=True
            )
...
def build_sim_config(config_data, overrides):
    """Build a SimulationConfig with the variant overrides applied."""
    simulation_parameters = dict(config_data["simulation_parameters"])
    simulation_parameters.update(overrides)
    return SimulationConfig(**simulation_parameters)


def run_variant(config_data, flight_data, overrides):
    """Run one full EKF simulation and return its outputs and calibrated input.
    ...
    """
    simConfig = build_sim_config(config_data, overrides)
    ...
    # Fresh inputs per variant: the calibration rewrites them in place
...
def unwrap_direction(wind_direction):
    """Continuous wind direction in degrees, robust to wrapping and NaN gaps."""
    degrees = np.rad2deg(wind_direction) % 360
    finite = np.isfinite(degrees)
    if finite.any():
        degrees[finite] = np.rad2deg(np.unwrap(np.deg2rad(degrees[finite])))
    return degrees
...
def plot_comparison(results, flight_data, kite_model, date, savepath):
    ...
    # Lidar profile as an independent reference
    ...
    for key, label, _ in VARIANTS:
        ...
        wind_direction = unwrap_direction(
        ...
        # What the calibration did to the pitot reading
        axs[1, 1].plot(
            results["measured_va"],
...
def main():
    ...
    for key, label, overrides in VARIANTS:
        ...
        print("=" * 74)
        outputs, calibrated_va = run_variant(config_data, flight_data.copy(), overrides)
