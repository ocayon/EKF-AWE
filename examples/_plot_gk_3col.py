...
def plot_yaw_rate_vs_steering(
    ...
    if len(x_fit) > 1:
        ...
        ax.plot(
            ...
            linestyle="--",
            linewidth=1.5,
            label=f"Exp. fit $g_\\textrm{{k}}={slope:.1f}$, R$^2$={r2:.2f}",
        )
    ...
    if circle_df is not None and not circle_df.empty:
        ...
        if finite_uniform.sum() > 1:
            ...
            uniform_label = f"Sim. uniform $g_\\textrm{{k}}={slope_uniform:.1f}$, R$^2$={r2_uniform:.2f}"
        ...
        if len(x_dyn_all) > 1 and len(y_dyn_all) > 1:
            ...
            dynamic_label = (
                f"Sim. dynamic $g_\\textrm{{k}}={slope_dyn:.1f}$, R$^2$={r2_dyn:.2f}"
            )
