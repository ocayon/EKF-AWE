...
def plot_kite_orientation(config_data: dict) -> None:
    ...
    # Calculate errors
    pitch_error = abs(flight_data['kite_pitch_s0']-results['kite_pitch'])
    ...
    pu.plot_time_series(flight_data,results['kite_pitch'],ax, label='Estimated',plot_phase=False)
    ax.grid()
    ax.legend()
    ...
    pu.plot_time_series(flight_data,results['kite_roll'], ax, label='Estimated',plot_phase=False)
    ax.grid()
    ax.legend()
    ...
    pu.plot_time_series(flight_data,results['kite_yaw'], ax, label='Estimated',plot_phase=False)
    ax.grid()
    ax.legend()
    
    fig,ax = plt.subplots()
    pu.plot_time_series(flight_data, results['kite_roll']-results['kcu_roll'], ax, label='Roll kite-tether',plot_phase=False)
    
    pu.plot_time_series(flight_data,results['kite_pitch']-results['kcu_pitch'], ax, label='Pitch kite-tether',plot_phase=True)
