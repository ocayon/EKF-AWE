...
# Your Kalman filter code here
        ...
        for k, ekf_input in enumerate(ekf_input_list):
            # cpu_usage=psutil.cpu_percent(interval=0.1, percpu=True)     #For all cores
            # average_cpu_usage.append(sum(cpu_usage) / len(cpu_usage))
            # print(f"Average CPU usage: {average_cpu_usage[-1]:.2f}%")
        ...
        # pr.disable()
        # print(f"Average CPU usage: {sum(average_cpu_usage)/len(average_cpu_usage):.2f}%")
        # s = StringIO()
    ...
    def run(self):
        ...
        self.run_analysis()
        after_memory = process.memory_info().rss / (1024 ** 2)  # In MB
    ...
    def track_memory_usage(self):
        ...
        before_memory = process.memory_info().rss / (1024 ** 2)  # In MB
        self.run_analysis()  # Replace with the function you want to analyze
        after_memory = process.memory_info().rss / (1024 ** 2)  # In MB
        ...
        # Optional: Get detailed traceback for the largest allocation
        for stat in snapshot.statistics("traceback")[:1]:
            print("\nDetailed traceback of the largest memory allocation:")
