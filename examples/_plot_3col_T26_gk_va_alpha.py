...
def load_vw8_varying_us_points(
    ...
    """Load VW8 points as (x=u_dp, y=metric) arrays using base CSV columns only."""
    ...
    u_dp_levels = np.array([0.22, 0.32, 0.42], dtype=float)
    u_dp = u_dp_levels[
        np.argmin(
    ...
    for metric, y_vals in points_raw.items():
        ...
        points[metric] = (
            grouped["u_dp"].to_numpy(dtype=float),
            grouped["y"].to_numpy(dtype=float),
        )
        print(
            f"Loaded VW8 overlay for {metric}: "
            f"n_raw={int(np.count_nonzero(finite))}, n_avg={len(grouped)}"
        )
...
def load_vw8_transient_average_points(
    ...
    """Load transient-harmonic averages (n=4..9) as open-circle overlay points."""
    if not csv_path.is_file():
        print(f"VW8 CSV not found: {csv_path}")
        return {}
...
def main() -> None:
    ...
    vw8_overlay = load_vw8_varying_us_points(
        Path("./data/vw8_lt270_circles_combined.csv")
    )
    transient_overlay = load_vw8_transient_average_points(
        Path("./data/vw8_lt270_circles_combined.csv")
    )
    ...
    legend_handles = [
        Patch(
            ...
            edgecolor=to_rgba(year_colors["2019"], alpha=1.0),
            hatch="///",
            linewidth=1.0,
            alpha=1.0,
            label="2019 upward",
        ...
        Patch(
            ...
            edgecolor=to_rgba(year_colors["2025"], alpha=1.0),
            hatch="///",
            linewidth=1.0,
            alpha=1.0,
            label="2025 upward",
