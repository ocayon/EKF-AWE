...
def process_KP_data(df):
    ...
    for sensor in sensors:
        # Velocity data
        flight_data["kite_velocity_east_s" + str(sensor)] = df[
            "kite_" + str(sensor) + "_vy"
        ]
        flight_data["kite_velocity_north_s" + str(sensor)] = df[
            "kite_" + str(sensor) + "_vx"
        ]
        flight_data["kite_velocity_up_s" + str(sensor)] = -df[
            "kite_" + str(sensor) + "_vz"
        ]
        # Euler angles data
        flight_data["kite_pitch_s" + str(sensor)] = np.deg2rad(
            df["kite_" + str(sensor) + "_pitch"]
        )
        flight_data["kite_roll_s" + str(sensor)] = np.deg2rad(
            df["kite_" + str(sensor) + "_roll"]
        )
        flight_data["kite_yaw_s" + str(sensor)] = np.deg2rad(
            df["kite_" + str(sensor) + "_yaw"]
        )
        # Acceleration data
    ...
    flight_data["ground_wind_direction"] = (
        360 - 90 - df["ground_upwind_direction"]
    ...
    flight_data["ground_tether_reelout_speed"] = df[
...
file_name.append("2019-10-08_11.csv")
