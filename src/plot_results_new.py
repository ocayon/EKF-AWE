...
pu.plot_wind_speed(results,flight_data, plot_lidar_heights,IMU_0=IMU_0, IMU_1=IMU_1, EKF_tether=EKF_tether) # PLot calculated wind speed against lidar
...
################## Time series ##################
cycles_plotted = np.arange(0,20,1)
...
#%%
plt.figure()
vw0_mod = np.sqrt(flight_data['vwx_IMU_0']**2+flight_data['vwy_IMU_0']**2)
vw_EKF = np.sqrt(flight_data['vwx_EKF']**2+flight_data['vwy_EKF']**2)
plt.plot(flight_data['time'],vw0_mod)
plt.plot(flight_data['time'],vw_EKF)

# plot wind direction
plt.figure()
vw0_dir = np.arctan2(flight_data['vwy_IMU_0'],flight_data['vwx_IMU_0'])*180/np.pi
vw_EKF_dir = np.arctan2(flight_data['vwy_EKF'],flight_data['vwx_EKF'])*180/np.pi
plt.plot(flight_data['time'],vw0_dir)
plt.plot(flight_data['time'],vw_EKF_dir)

# plot vertical wind speed
plt.figure()
plt.plot(flight_data['time'],flight_data['vwz_IMU_0'])
plt.plot(flight_data['time'],flight_data['vwz_EKF'])
