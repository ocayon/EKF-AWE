...
def calculate_yaw_rate_new(coeffs, area,mass,span, us, va, beta, yaw,v,radius,offset_cg = True, turn_rate_equal = True):
    ...
    if turn_rate_equal:
        yaw_rate = area*Cn_us*us*(va**2)/(v*mass)+Cn_i*us*(va)/(span**2)
    else:
        yaw_rate = Cn_us*us*(va)/span
        if offset_cg:
            yaw_rate += Cn_i *( mass*v**2/(rho*area*span**2*va*radius)+mass*gravity_constant*np.sin(yaw)*np.cos(beta)/(rho*area*span**2*va))
            
    return yaw_rate


def calculate_yaw_rate(coeffs, kite, us, va, beta, yaw,v,radius,forces):
...
def obj_yaw_rate(x,area,mass,span,us_data, va_data, beta,yaw,v_kite,radius, observed_yaw_rates,offset_cg = True, turn_rate_equal = True):
    estimated_yaw_rates = calculate_yaw_rate_new(x,area,mass,span,us_data, va_data, beta, yaw,v_kite,radius,offset_cg, turn_rate_equal)
