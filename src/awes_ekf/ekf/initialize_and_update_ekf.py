...
def initialize_ekf(ekf_input, model_specs, system_specs,x0):
    """
...
def update_state_ekf_tether(ekf, tether, kite, kcu, ekf_input, model_specs):
    """
...
def propagate_state_EKF(ekf, dyn_model, ekf_input, model_specs, tether, kite, kcu):
    # Predict step
    ekf.x_k1_k = dyn_model.propagate(ekf.x_k1_k1,ekf.u, kite, tether, kcu, ekf_input.ts)

    ## Update step
    ekf, ekf_ouput = update_state_ekf_tether(ekf, tether, kite, kcu, ekf_input, model_specs)
