...
def plot_identify_turn_dynamics(results, flight_data, config_data, title=""):
    # Turn rate law
    ...
    yaw_rate_standard, coeffs_standard = calculate_turn_rate_law(
        results, flight_data, model="simple", steering_offset=False
    )
    yaw_rate_offset_corrected, coeffs_offset = calculate_turn_rate_law(
    ...
    error_standard = abs(
        np.degrees(yaw_rate_standard) - np.degrees(flight_data["kite_yaw_rate"])
    )
    error_offset = abs(
        np.degrees(yaw_rate_offset_corrected) - np.degrees(flight_data["kite_yaw_rate"])
    )
    ...
    A = np.vstack([x]).T
    y_identified_standard = np.degrees(A @ coeffs_standard)
    plt.plot(
        x,
    ...
    y_identified_offset = np.degrees(A @ coeffs_offset)
    plt.plot(
        x,
        y_identified_offset,
    ...
    plt.ylabel("Kite Yaw Rate [deg/s]")
    plt.legend()
    plt.tight_layout()
    ...
    axs[1].plot(
        flight_data["time"],
        np.degrees(yaw_rate_standard),
        label="Identified Yaw Rate",
        linestyle="--",
    )
    axs[1].plot(
        flight_data["time"],
        np.degrees(yaw_rate_offset_corrected),
        label="Offset-Corrected Yaw Rate",
        linestyle="-.",
    )
