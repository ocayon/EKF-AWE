...
from awes_ekf.plotting.plot_utils import plot_kinetic_energy_spectrum
...
def plot_wind_velocity(results, flight_data, config_data):
    ...
    plot_kinetic_energy_spectrum(results, flight_data, ax=ax)
    plt.show()
...
def plot_wind_timeseries(results, flight_data):
    ...
    axs[1].plot(
        flight_data["time"],
        results["wind_direction"] * 180 / np.pi,
        label="EKF",
        color=colors[0],
    )
    ...
    if "interp_wind_speed" in flight_data.columns:
        ...
        axs[1].plot(
            flight_data["time"],
            270 - flight_data["interp_wind_direction"],
            label="Lidar",
            color=colors[1],
        )
        axs[2].plot(
            flight_data["time"],
            flight_data["interp_z_wind"],
            label="Lidar",
            color=colors[1],
        )
...
def interpolate_lidar_data(flight_data, results):
    ...
    for index in flight_data.index:
        ...
        for height in lidar_heights:
            wind_speeds.append(
                flight_data[str(height) + "m_Wind_Speed_m_s"].iloc[index]
            )
            wind_speeds_max.append(
                flight_data[str(height) + "m_Wind_Speed_max_m_s"].iloc[index]
            )
            wind_speeds_min.append(
                flight_data[str(height) + "m_Wind_Speed_min_m_s"].iloc[index]
            )
            wind_directions.append(
                flight_data[str(height) + "m_Wind_Direction_deg"].iloc[index]
            )
...
def log_interp(height, heights, values):
    ...
    if height <= heights.min():
        ...
        return u1 + (np.log(height) - np.log(z1)) / (np.log(z2) - np.log(z1)) * (
            u2 - u1
        )
