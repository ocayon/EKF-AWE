...
def plot_kinematics(results, flight_data, config_data):
    ...
    r = np.sqrt(
        results["kite_position_x"] ** 2
        + results["kite_position_y"] ** 2
        + results["kite_position_z"] ** 2
    )
    axs[1].plot(flight_data["time"], r, label="EKF", linewidth=1)
    r = np.sqrt(
        flight_data["kite_position_x"] ** 2
        + flight_data["kite_position_y"] ** 2
        + flight_data["kite_position_z"] ** 2
    )
    ...
    meas_kite_speed = np.sqrt(
        flight_data["kite_velocity_x"] ** 2
        + flight_data["kite_velocity_y"] ** 2
        + flight_data["kite_velocity_z"] ** 2
    )
    ...
    plt.plot(
        ...
        label="EKF apparent wind speed",
    )
    try:
        plt.plot(
            flight_data["time"],
            flight_data["kite_apparent_windspeed"],
            label="Measured apparent wind speed",
        )
    except:
        pass
    plt.xlabel("Time [s]")
...
def plot_kite_orientation(results, flight_data, config_data):
    ...
    plot_time_series(
        flight_data,
        np.degrees(results["kite_roll"]) - 360,
        axs[0],
        label="EKF 1",
        plot_phase=False,
    )
    ...
    plot_time_series(
        flight_data,
        np.degrees(results["kite_yaw"]),
        axs[2],
        label=r"Aligned with $v_\mathrm{a}$",
        plot_phase=False,
    )
    plot_time_series(
        flight_data,
        np.degrees(results["kite_yaw_kin"]),
        axs[2],
        label=r"Aligned with $v_\mathrm{k}$",
        plot_phase=False,
        color=colors[3],
    )
